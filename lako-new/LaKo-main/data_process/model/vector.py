    ...
    def cache(self, name, cache_path, url=None):
        ...
        if not osp.isfile(path_pt):
            ...
            if not osp.exists(path) and url:
                ...
                if not osp.exists(dest):  # If this zip file doesn't exist
                    print('[-] Downloading vectors from {}'.format(url))  # Download
                    if not osp.exists(cache_path):
                        os.mkdir(cache_path)  # Create this cache folder

                    with tqdm(unit='B', unit_scale=True, miniters=1, desc=dest) as t:
                        urlretrieve(url, dest, reporthook=reporthook(t))  # Download

                print('[-] Extracting vectors into {}'.format(path))  # Extract vectors to txt
                ext = os.path.splitext(dest)[1][1:]
                if ext == 'zip':
                    with zipfile.ZipFile(dest, "r") as zf:
                        zf.extractall(cache_path)  # Extract
            ...
            print("[-] Loading vectors from {}".format(path))  # Read vectors
            for line in tqdm(lines, total=len(lines)):
                ...
                if dim is None and len(entries) > 1:
                    dim = len(entries)  # dim is determined by vector length
                elif len(entries) == 1:
                    print("Skipping token {} with 1-dimensional "
                          "vector {}; likely a header".format(word, entries))
                    continue
                elif dim != len(entries):  # Vector lengths are not equal
                    raise RuntimeError(
                        "Vector for token {} has {} dimensions, but previously "
                        "read vectors have {} dimensions. All vectors must have "
                        "the same number of dimensions.".format(word, len(entries), dim))

                vectors.extend(float(x) for x in entries)  # Convert vectors to float and store in vector
                itos.append(word)  # Add word to index to string

            self.itos = itos
            self.stoi = {word: i for i, word in enumerate(itos)}  # string to index
            self.vectors = torch.Tensor(vectors).view(-1, dim)  # Convert to tensor
            self.dim = dim
            print('* Caching vectors to {}'.format(path_pt))  # Save to pt file
            torch.save((self.itos, self.stoi, self.vectors, self.dim), path_pt)
        else:  # If pt file exists, read directly
