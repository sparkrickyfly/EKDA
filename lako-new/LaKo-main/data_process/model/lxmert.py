    ...
    def __init__(self, args, num_labels=None):
        ...
        if len(args.load) <= 1 and not args.split_segment:
            ...
            if args.split_segment:
                # embedding modification makes finetuning meaningless
                assert not args.finetune
                config_class = LxmertConfig(type_vocab_size=2)  # 2 is enough? Only determines the size of type_vocab in neural network
            else:
